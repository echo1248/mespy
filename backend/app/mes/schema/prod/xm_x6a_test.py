...
class XMX6ATestSchemaBase(SchemaBase):
    """x6a产品基础模型"""
    ...
    test_snkeyold: str | None = Field(default=None, description='旧的SN键')
    test_subkey: str | None = Field(default=None, description='绑定的子件物料')
    test_subtypekey: str | None = Field(default=None, description='绑定的子件物料类型编码')
    test_submtrlkey: str | None = Field(default=None, description='子件物料编码')
...
class CreateXMX6ATestParam(XMX6ATestSchemaBase):
    """创建x6a产品测试参数"""


class UpdateXMX6ATestParam(XMX6ATestSchemaBase):
    """更新x6a产品测试参数"""


class DeleteXMX6ATestParam(SchemaBase):
    """删除x6a产品测试参数"""

    pks: list[int] = Field(description='ID列表')


class GetXMX6ATestDetail(XMX6ATestSchemaBase):
    """x6a产品测试详情"""
