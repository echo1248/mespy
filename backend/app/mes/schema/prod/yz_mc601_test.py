...
class YZMC601TestSchemaBase(SchemaBase):
    ...
    test_activated: bool = Field(default=False, description='芯片激活标记: 0-未激活, 1-已激活')
    ...
    test_stkey: str = Field(description='站键')
    test_sttitle: str | None = Field(default=None, description='站标题')
    test_stord: int | None = Field(default=None, description='站顺序')
    ...
    test_printlock: bool = Field(default=False, description='打印锁定状态: 0-未锁定, 1-锁定')
    ...
    test_pass: bool = Field(default=False, description='总体测试通过状态: 0-失败, 1-通过')
    test_key_gened: bool = Field(default=False, description='未使用该栏位')
