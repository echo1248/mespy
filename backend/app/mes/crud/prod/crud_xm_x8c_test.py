...
class CRUDXMX8CTestDao(CRUDPlus[XMX8CTest]):

    async def get(self, db: AsyncSession, pk: int) -> XMX8CTest | None:
    ...
    async def create(self, db: AsyncSession, obj: CreateXMX8CTestParam) -> None:
        await self.create_model(db, obj)

    async def bulk_create(self, db: AsyncSession, objs: list[CreateXMX8CTestParam]) -> None:
    ...
    async def delete_all(db: AsyncSession) -> None:
        await db.execute(sa_delete(XMX8CTest))


xm_x8c_test_dao: CRUDXMX8CTestDao = CRUDXMX8CTestDao(XMX8CTest)
