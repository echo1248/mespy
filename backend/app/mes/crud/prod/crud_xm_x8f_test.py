...
class CRUDXMX8FTestDao(CRUDPlus[XMX8FTest]):

    async def get(self, db: AsyncSession, pk: int) -> XMX8FTest | None:
    ...
    async def create(self, db: AsyncSession, obj: CreateXMX8FTestParam) -> None:
        await self.create_model(db, obj)

    async def bulk_create(self, db: AsyncSession, objs: list[CreateXMX8FTestParam]) -> None:
    ...
    async def delete_all(db: AsyncSession) -> None:
        await db.execute(sa_delete(XMX8FTest))


xm_x8f_test_dao: CRUDXMX8FTestDao = CRUDXMX8FTestDao(XMX8FTest)
