...
class CRUDXMOH2TestDao(CRUDPlus[XMOH2Test]):

    async def get(self, db: AsyncSession, pk: int) -> XMOH2Test | None:
    ...
    async def create(self, db: AsyncSession, obj: CreateXMOH2TestParam) -> None:
        await self.create_model(db, obj)

    async def bulk_create(self, db: AsyncSession, objs: list[CreateXMOH2TestParam]) -> None:
    ...
    async def delete_all(db: AsyncSession) -> None:
        await db.execute(sa_delete(XMOH2Test))


xm_oh2_test_dao: CRUDXMOH2TestDao = CRUDXMOH2TestDao(XMOH2Test)
