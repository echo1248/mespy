...
class CRUDXMXHBTestDao(CRUDPlus[XMXHBTest]):

    async def get(self, db: AsyncSession, pk: int) -> XMXHBTest | None:
    ...
    async def create(self, db: AsyncSession, obj: CreateXMXHBTestParam) -> None:
        await self.create_model(db, obj)

    async def bulk_create(self, db: AsyncSession, objs: list[CreateXMXHBTestParam]) -> None:
    ...
    async def delete_all(db: AsyncSession) -> None:
        await db.execute(sa_delete(XMXHBTest))


xm_xhb_test_dao: CRUDXMXHBTestDao = CRUDXMXHBTestDao(XMXHBTest)
