...
class XMX6ATest(DataClassBase):
    """x6a产品表"""

    __tablename__ = 'xm_x6a_test'

    __table_args__ = (
        # 唯一索引
        Index('test_stkey_snkey_index', 'test_stkey', 'test_snkey', 'test_times_putin', unique=True),
        Index('test_stkey_subkey_index', 'test_stkey', 'test_subkey', 'test_times_putin', unique=True),

        # 普通索引
        Index('test_stkey_index', 'test_stkey'),
        Index('test_snkey_index', 'test_snkey'),
        Index('test_subkey_idnex', 'test_subkey'),
        Index('test_subtypekey_index', 'test_subtypekey'),
        Index('test_submtrlkey_index', 'test_submtrlkey'),
        ...
        Index('test_times_putin_index', 'test_times_putin'),
    ...
    test_snkeyold: Mapped[str | None] = mapped_column(CHAR(32), comment='旧的SN键')  # 新增字段
    test_subkey: Mapped[str | None] = mapped_column(CHAR(64), comment='绑定的子件物料')
    test_subtypekey: Mapped[str | None] = mapped_column(CHAR(32), comment='绑定的子件物料类型编码')  # 新增字段
    test_submtrlkey: Mapped[str | None] = mapped_column(CHAR(32), comment='子件物料编码')  # 新增字段
