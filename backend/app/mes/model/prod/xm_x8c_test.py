...
from sqlalchemy import Boolean, DateTime, Integer, CHAR, VARCHAR, TEXT, TIMESTAMP, Index, INTEGER
...
class XMX8CTest(DataClassBase):
    """x8c产品表"""

    __tablename__ = 'xm_x8c_test'
    ...
    test_deleted: Mapped[bool] = mapped_column(Boolean().with_variant(INTEGER, 'postgresql'),
                                               comment='是否删除')
    test_createdon: Mapped[datetime | None] = mapped_column(TIMESTAMP, server_default='CURRENT_TIMESTAMP',
                                                            comment='创建时间')
    ...
    test_info_1: Mapped[str | None] = mapped_column(TEXT, comment='第1次测试信息')
    test_info_2: Mapped[str | None] = mapped_column(TEXT, comment='第2次测试信息')
    test_info_3: Mapped[str | None] = mapped_column(TEXT, comment='第3次测试信息')
